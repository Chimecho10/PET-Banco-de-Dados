...
export default function Aba_Participantes({ voltarAbaLobby }) {
  ...
  // LISTA DE PARTICIPANTES QUE VEM DO BANCO DE DADOS
  ...
  const editartrue = (user) => {
    setId_part(user.id);
    setUsername_edicao(user.username || user.user || '');
    setNome_edicao(user.nome || '');
    setCpf_edicao(user.cpf ? user.cpf.replace(/\D/g, '') : '');
  ...
  const deletartrue = (user) => {
    ...
    setUsername_deletando(user.username || user.user || '');
  ...
  const deletarPart = async (e) => {
    e.preventDefault();
    try {
      await deletarParticipanteAPI(id_deletando);
      carregarParticipantesAPI(setListaDeTeste);
    } catch (error) {
      console.error(error);
    }
  ...
  const adicionarparticipante = async (e) => {
    ...
    // CORREÇÃO 2: Evita o erro 'toLowerCase of undefined'
    if (listaDeTeste.find((user) => user.cpf ? user.cpf.replace(/\D/g, '').includes(cpf) : false)) {
      alert("CPF já consta nos participantes");
      return;
    }
    if (listaDeTeste.find((user) => (user.username || user.user)?.toLowerCase().includes(username.toLowerCase()))) {
      alert("Usuário já consta nos participantes");
      return;
    }
    const camposEnvio = { username, nome, cpf };
    const funcoesEnvio = { setJanelaEditar: setJanelaAdicionar, setUsername, setCpf, setNome };

    try {
      await enviarParticipante(e, camposEnvio, funcoesEnvio);
      carregarParticipantesAPI(setListaDeTeste);
      setJanelaAdicionar(false);
    } catch (error) {
      console.error(error);
  ...
  const procurarParticipantes = (e) => {
    ...
    if (nomeoucpf !== '') {
      ...
      const busca = listaDeTeste.filter((user) => {
        const achouNome = user.nome ? user.nome.toLowerCase().includes(nomeoucpf.toLowerCase()) : false;
        const achouCpf = user.cpf ? user.cpf.replace(/\D/g, '').includes(nomeoucpf.toLowerCase()) : false;
      ...
      <div className='megablocored'
        ...
        <div style={{
          ...
          <button className='buttonvlt2'
            ...
            <div className='buttonvoltando'>Voltar</div>
          </button>

          <h1 className='letreiro5' style={{ margin: '0 auto' }}>PARTICIPANTES</h1>

          <div className='procurando2' style={{ userSelect: 'none', marginLeft: '200px' }}>
            <form onSubmit={procurarParticipantes}
              ...
              <button type='submit'
                ...
                <img src={lupa} style={{ marginRight: '10px', width: '38px', }} />
              </button>
        ...
        <div className='overlay'>
          <div className='trymodal' style={{ display: 'flex', flexDirection: 'column', }}>
            <button onClick={desaddpartrue} className='buttonvlt'>
              <img style={{ width: '25px' }} src={sair} alt="Sair" />
            </button>

            <h1 className='letreiro3' style={{ userSelect: 'none' }}>ADICIONAR PARTICIPANTE</h1>
      ...
      {janelaEditar &&
        <div className='overlay'>
          <div className='trymodal' style={{ display: 'flex', flexDirection: 'column', }}>
            ...
            <h1 className='letreiro3' style={{ userSelect: 'none' }}>EDITAR PARTICIPANTE</h1>
            ...
            <form onSubmit={editarPart}
              ...
              <div className='retanguloamarelo'>
                ...
                <input type="text"
                  ...
                  onChange={(e) => setUsername_edicao(e.target.value)}
                  className='campoadd' />
              </div>
              <div className='retanguloamarelo'>
                <h1 className='letreiro6'>Nome:</h1>
                <input type="text"
                  value={nome_edicao}
                  onChange={(e) => setNome_edicao(e.target.value)}
                  className='campoadd' />
              </div>
              <div className='retanguloamarelo'>
                <h1 className='letreiro6'>CPF:</h1>
                <input type="text"
                  ...
                  onChange={(e) => {
                    const dig = e.target.value;
                    const tig = dig.replace(/\D/g, '');
                    if (tig.length <= 11) { setCpf_edicao(tig) }
                  }}
                  className='campoadd' />
              </div>
              <button type='submit' className='buttonenviar' style={{ marginTop: '20px' }}>
      ...
      {janelaDeletar &&
        <div className='overlay'>
          <div className='trymodal' style={{ display: 'flex', flexDirection: 'column', }}>
            ...
            <h1 className='letreiro3' style={{ userSelect: 'none' }}>DELETAR PARTICIPANTE</h1>
